                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p><strong>Name:</strong> {{ $quotation->inquiry->first_name }} {{ $quotation->inquiry->last_name }}</p>
                                ...
                                <div class="row">
                                    <div class="col-md-8">
                                        ...
                                        @if(is_array($day->meals_included) && count($day->meals_included) > 0)
                                            <p><strong>Meals Included:</strong> {{ implode(', ', $day->meals_included) }}</p>
                                        @endif
                                    ...
                                    <div class="col-md-4">
                                        <div class="text-right">
                                            <h6 class="text-success">Cost per Person: {{ $quotation->currency }} {{ number_format($day->cost_per_person, 2) }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        @foreach($quotation->events as $event)
                            <div class="event-detail mb-3 p-3 border rounded {{ $event->is_optional ? 'border-warning' : '' }}">
                                <div class="row">
                                    <div class="col-md-8">
                                        <h6>{{ $event->event_name }}
                                    ...
                                    <div class="col-md-4">
                                        <div class="text-right">
                                            <h6 class="text-success">Cost per Person: {{ $quotation->currency }} {{ number_format($event->cost_per_person, 2) }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        @foreach($quotation->vehicles as $quotationVehicle)
                            <div class="vehicle-detail mb-3 p-3 border rounded">
                                <div class="row">
                                    <div class="col-md-8">
                                        <h6>{{ $quotationVehicle->vehicle->name }} ({{ $quotationVehicle->vehicle->type }})</h6>
                                        <p><strong>Capacity:</strong> {{ $quotationVehicle->vehicle->capacity }} people</p>
                                        <p><strong>Days Assigned:</strong> {{ implode(', ', $quotationVehicle->days_assigned) }}</p>
                                        <p><strong>Route:</strong> {{ $quotationVehicle->pickup_location }} → {{ $quotationVehicle->dropoff_location }}</p>
                                        <p><strong>Estimated Distance:</strong> {{ $quotationVehicle->estimated_km }} KM</p>
                                        <p><strong>Driver:</strong> {{ $quotationVehicle->driver_required ? 'Required' : 'Not Required' }}</p>
                                        @if($quotationVehicle->special_requirements)
                                            <p><strong>Special Requirements:</strong> {{ $quotationVehicle->special_requirements }}</p>
                                        @endif
                                    </div>
                                    <div class="col-md-4">
                                        <div class="text-right">
                                            <p><strong>Cost per Day:</strong> {{ $quotation->currency }} {{ number_format($quotationVehicle->cost_per_day, 2) }}</p>
                                            <h6 class="text-success">Total Cost: {{ $quotation->currency }} {{ number_format($quotationVehicle->total_cost, 2) }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-8">
                            <table class="table">
                                <tr>
                                    ...
                                    <td class="text-right">{{ $quotation->currency }} {{ number_format($quotation->days->sum('cost_per_person') * $quotation->inquiry->group_size, 2) }}</td>
                                </tr>
                                @if($quotation->events->count() > 0)
                                <tr>
                                    <td><strong>Events Cost:</strong></td>
                                    <td class="text-right">{{ $quotation->currency }} {{ number_format($quotation->events->sum('cost_per_person') * $quotation->inquiry->group_size, 2) }}</td>
                                </tr>
                                @endif
                                @if($quotation->vehicles->count() > 0)
                                <tr>
                                    <td><strong>Vehicles Cost:</strong></td>
                                    <td class="text-right">{{ $quotation->currency }} {{ number_format($quotation->vehicles->sum('total_cost'), 2) }}</td>
                                </tr>
                                @endif
                        ...
                        <div class="col-md-4">
                            <div class="text-right">
                                <h5 class="text-success">Total Cost: {{ $quotation->currency }} {{ number_format($quotation->total_amount, 2) }}</h5>
                            </div>
                        </div>
                    </div>
                </div>
